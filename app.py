...
def analyze_exoplanet(files, sde_thresh=7.5):
    ...
    if len(time_all) > 100:
        ...
        ax1.plot(t_smooth, f_int(t_smooth), 'cyan', lw=1.2, alpha=0.9)
...
# SIMPLE SPRAVKA
terms = {
    "SDE": "Signal strength >7.5=strong candidate",
    "BLS": "Transit search algorithm",
    "PDCSAP": "Clean pipeline flux (recommended)",
    "SAP": "Raw flux",
    "LLC": "30min cadence",
    "SLC": "1min cadence", 
    "AI": "Neural net planet probability >70%=confirmed"
}
...
def search_help(query):
    ...
    for k in terms:
        if q in k.lower():
            return f"**{k}**: {terms[k]}"
    return list(terms.keys())[0] + ': ' + terms[list(terms.keys())[0]]

with gr.Blocks(css=css) as app:
    gr.Markdown('# Exoplanet Finder')

    with gr.Row():
        files = gr.File(file_count='multiple', file_types=['.fits'], label='FITS files')
        sde_slider = gr.Slider(5, 15, 7.5, step=0.1, label='SDE threshold')

    analyze_btn = gr.Button('Analyze + AI', variant='primary')

    result_text = gr.Textbox(label='BLS + AI Results', lines=15)
    result_img = gr.Image(label='Dark Plots', type='pil')

    # SPRAVKA ACCORDION (NO ERRORS)
    with gr.Accordion("📚 Help", open=False):
        search_box = gr.Textbox(label="Search term (SDE, BLS, AI...)", placeholder="Enter term")
        help_display = gr.Markdown()
        search_box.submit(search_help, search_box, help_display)
